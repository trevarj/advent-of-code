    ...
    (pcase type
      ...
      (?\[
       (-when-let* ((moved (try-move map next dir))
                    ((side &as sx sy) (add-pairs obj (dir-to-vec :right)))
                    ((side-next &as snx sny) (add-pairs next (dir-to-vec :right)))
                    (side-moved (try-move map side-next dir)))
         (setf-grid map x y ?.)
         (setf-grid map nx ny ?\[)
         (setf-grid map sx sy ?.)
         (setf-grid map snx sny ?\])
         t))
      (?\]
       (-when-let* ((moved (try-move map next dir))
                    ((side &as sx sy) (add-pairs obj (dir-to-vec :left)))
                    ((side-next &as snx sny) (add-pairs next (dir-to-vec :left)))
                    (side-moved (try-move map side-next dir)))
         (setf-grid map x y ?.)
         (setf-grid map nx ny ?\])
         (setf-grid map sx sy ?.)
